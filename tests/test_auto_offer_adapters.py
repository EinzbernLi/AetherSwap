...
def test_request_and_result_are_immutable_and_preserve_exact_identity():
    original = request()
    result = PlatformResult(original, PlatformResultStatus.SUCCESS)
...
def test_fake_adapter_is_deterministic_for_the_same_exact_request():
    item = request()
    adapter = FakePlatformAdapter(
...
def test_malformed_and_identity_mismatched_results_fail_closed():
    ...
    adapter = FakePlatformAdapter(
        outcomes={
            item: object(),
            wrong_identity: PlatformResult(item, PlatformResultStatus.SUCCESS),
        }
    )
