...
class HistoryClient:
    ...
    def get_steam_trades(self):
        self.steam_calls += 1
        if isinstance(self.steam_trades, BaseException):
            raise self.steam_trades
...
def test_coordinator_result_unknown_does_not_use_history_after_realtime_failure():
    ...
    store = SpyStore(before)
    client = HistoryClient(history_pages={1: history_page(items=[history_record()])})
    client.steam_trades = RuntimeError("network")
    ...
    coordinator = DeliveryCoordinator(
        ...
        timeout_seconds=5.0,
        clock=lambda: (_ for _ in ()).throw(
            AssertionError("failed recovery must not read the clock")
        ),
    )
